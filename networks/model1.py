...
class Encoder(nn.Module):
    def __init__(self, n_channels=1, n_filters=16, has_residual=False):
        ...
        self.blk1 = DualDomainBlockV2(n=stages[1], channels=chs[1], num_heads=4)

        self.down1 = DownsamplingConvBlock(chs[1], chs[2])  # 32->64
        self.blk2 = DualDomainBlockV2(n=stages[2], channels=chs[2], num_heads=8)

        self.down2 = DownsamplingConvBlock(chs[2], chs[3])  # 64->128
        self.blk3 = DualDomainBlockV2(n=stages[3], channels=chs[3], num_heads=8)
...
class Decoder(nn.Module):
    def __init__(self, n_classes=14, n_filters=16, has_residual=False):
        ...
        self.blocks = nn.ModuleList([
            ...
            conv(dec_stages[3], 2 * chs[0], chs[0]),      # up(32->16)   cat skip(16)
        ])
